#!/usr/bin/env python3
"""Pluggable credential storage for cloudtalk-mcp.
...
Storage order (read):  OS keyring  ->  process env  ->  ~/.cloudtalk-mcp/.env file
...
user opts out of keyring with ``CLOUDTALK_MCP_USE_KEYRING=0``.
...
SERVICE_NAME = "cloudtalk-mcp"
CONFIG_DIR = Path.home() / ".cloudtalk-mcp"
ENV_FILE = CONFIG_DIR / ".env"
_USE_KEYRING_FLAG = "CLOUDTALK_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``CLOUDTALK_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
